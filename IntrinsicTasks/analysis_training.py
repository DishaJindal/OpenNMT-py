...
plt.plot([50*i for i in x], Task1, label="Simple Relabeling", marker=".", color='y')
if(int(sys.argv[2])>1):
  plt.plot([50*i for i in x], Task2, label="Complex Relabeling", marker=".", color='r')
if(int(sys.argv[2])>2):
...
plt.tight_layout()
plt.legend()
plt.savefig('figs/'+sys.argv[1]+'_training.png')
